...
if ($isAuthed) {
    ...
    $flagResult = $conn->query('SELECT f.flag, COUNT(*) AS duplicate_count, GROUP_CONCAT(c.title ORDER BY c.sort_order SEPARATOR ", ") AS titles, GROUP_CONCAT(c.challenge_id ORDER BY c.sort_order SEPARATOR ", ") AS challenge_ids FROM challenge_flags f LEFT JOIN challenges c ON c.challenge_id = f.challenge_id GROUP BY f.flag HAVING COUNT(*) > 1 ORDER BY duplicate_count DESC, f.flag ASC');
    if ($flagResult) while ($row = $flagResult->fetch_assoc()) $duplicateFlags[] = $row;
  ...
  <body>
    ...
    <main class="admin-page">
      ...
      <?php else: ?>
        ...
        <section class="admin-tabs" aria-label="admin actions"><a class="ghost-button <?= $editingChallenge ? '' : 'active-tab' ?>" href="/admin/">새 작전 과제</a><a class="ghost-button <?= $editingChallenge ? 'active-tab' : '' ?>" href="<?= $editingChallenge ? '/admin/?edit=' . h($editingChallenge['challenge_id']) : '#challenge-list' ?>">기존 과제 수정</a><a class="ghost-button" href="#challenge-list">과제 목록 / 삭제</a><a class="ghost-button" href="#team-list">전과 현황 팀원</a><a class="ghost-button" href="#flag-audit">플래그 중복 점검</a></section>
        ...
        <section class="challenge-detail admin-list admin-flag-audit" id="flag-audit"><div class="detail-header"><div><h2>플래그 중복 점검</h2><div class="meta"><span class="pill"><?= count($duplicateFlags) ?> duplicated</span><span class="pill">같은 인증 표식 확인</span></div></div></div><div class="admin-card-list"><?php if (!$duplicateFlags): ?><p class="form-help">중복된 플래그가 없습니다.</p><?php endif; ?><?php foreach ($duplicateFlags as $dup): ?><article class="admin-challenge-card"><div><strong><?= h($dup['flag']) ?></strong><small><?= h($dup['titles'] ?? '') ?></small><code><?= h($dup['challenge_ids'] ?? '') ?></code></div><div class="admin-card-meta"><span class="pill solved"><?= (int) $dup['duplicate_count'] ?>개 과제</span></div></article><?php endforeach; ?></div></section>
      <?php endif; ?>
    </main>
  </body>
</html>